...
/*   Updated: 2025/03/06 10:22:50 by yel-guad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_burningship(t_fract *my)
{
	...
	my->d.img = 0;
	my->mlx = mlx_init();
	my->win = mlx_new_window(my->mlx, 900, 900, "Burningship set");
...
int	draw_burningship(t_fract *my, int x, int y)
{
	...
	while (i < my->iter)
	{
		if (zp.real * zp.real + zp.img * zp.img > 4)
			return (my_pixel_put(my, x, y, get_color(i, my->iter, my)), 1);
		zn.real = fabs(zp.real * zp.real - zp.img * zp.img + my->c.real);
		zn.img = 2 * fabs(zp.real * zp.img) + my->c.img;
		zp = zn;
		i++;
	}
